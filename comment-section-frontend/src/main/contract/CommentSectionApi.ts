...
import { computeConcatMessage } from "./CommentSectionGenerated";
 ...
 */
export class CommentSectionApi {
  ...
  readonly compute = () => {
    ...
    const rpc = computeConcatMessage();
   ...
   */
  private readonly buildAddMessageRpc = (amount: number): Buffer => {
    // First build the public inputs
    const fnBuilder = new FnRpcBuilder("add_message", this.abi);
    const additionalRpc = fnBuilder.getBytes();

    // Then build the secret input
    const secretInputBuilder = ZkInputBuilder.createZkInputBuilder("add_message", this.abi);
