...
import { deserializeContractState } from "./contract/CommentSectionGenerated";
...
export const updateContractState = () => {
  ...
  CLIENT.getContractData<RawZkContractData>(address).then((contract) => {
    if (contract != null) {
      ...
      const concatMessageResult = <HTMLElement>document.querySelector("#average-result");
      concatMessageResult.innerHTML = `${state.concatMessageResult ?? "None"}`;

      const numComments = <HTMLElement>document.querySelector("#num-employees");
      numComments.innerHTML = `${state.numComments ?? "None"}`;
